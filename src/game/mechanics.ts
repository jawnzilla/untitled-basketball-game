...
const SHOT_SKILL_BASE = [0.48, 0.5, 0.52, 0.55, 0.58, 0.61, 0.64, 0.68, 0.72, 0.76, 0.8]
...
export function evaluateShot(ctx: ShotContext, random = Math.random): ShotDecision {
  ...
  const onFireBonus = ctx.isOnFire ? 0.12 + 0.12 * ctx.tuning.onFireBoost : 0

  const quality = clamp(skill - distancePenalty - threePenalty - defensePenalty + onFireBonus, 0.05, 0.97)
  const made = random() < quality

  const varianceScalar = 0.4 + 0.8 * ctx.tuning.shotVariance
  const missSpread = made ? 0.035 * varianceScalar : 0.32 * varianceScalar
...
export function evaluateDunk(ctx: DunkContext, random = Math.random): DunkDecision {
  ...
  const dunkSkillBonus = clamp(ctx.attacker.dunkSkill / 10, 0, 1) * (0.22 + 0.2 * ctx.tuning.dunkFrequency)
